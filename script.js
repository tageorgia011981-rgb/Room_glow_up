...
const catalogData = {
    seating: { 
        'couches': Array(5).fill({ img: 'couch1.png' }), 
        'armchairs': Array(5).fill({ img: 'chair1.png' }), 
        'dining chairs': Array(5).fill({ img: 'chair1.png' }), 
        'stools': Array(5).fill({ img: 'chair1.png' }) 
    },
    tables: { 
        'coffee': Array(5).fill({ img: 'table1.png' }), 
        'dining': Array(5).fill({ img: 'table2.png' }), 
        'side': Array(5).fill({ img: 'table1.png' }), 
        'desks': Array(5).fill({ img: 'table2.png' }) 
    },
    storage: { 
        'shelves': Array(5).fill({ img: 'shelf1.png' }), 
        'cabinets': Array(5).fill({ img: 'cabinet_item.png' }), 
        'dressers': Array(5).fill({ img: 'shelf1.png' }) 
    },
    kitchen: { 
        'stools': Array(5).fill({ img: 'chair1.png' }), 
        'lighting': Array(5).fill({ img: 'lamp1.png' }), 
        'runners': Array(5).fill({ img: 'rug1.png' }), 
        'decor': Array(5).fill({ img: 'plant1.png' }), 
        'islands': Array(5).fill({ img: 'table2.png' }) 
    }
};

const decorData = { 
    'Lighting': Array(5).fill({ img: 'lamp1.png' }), 
    'Rugs': Array(5).fill({ img: 'rug1.png' }), 
    'Art': Array(5).fill({ img: 'art1.png' }), 
    'Curtains': Array(5).fill({ img: 'curtain1.png' }), 
    'Plants': Array(5).fill({ img: 'plant1.png' }) 
};
...
function addToRoom(src) {
    ...
    // Special layering: Rugs and Runners stay behind furniture
...
function openMode(mode, el) {
    ...
    if (mode === 'rooms' || mode === 'furniture') {
        ...
        items.forEach(name => {
            ...
            let action = mode === 'rooms' 
                ? (name === 'Upload' ? "triggerUpload()" : `selectRoom('${file}', this)`) 
                : `selectRoom('room.jpg', this); showSubtypes('${name}')`;
...
function selectRoom(src, el) {
    ...
    setTimeout(() => {
        ...
        // Populate the 5 category blades (including Kitchen)
